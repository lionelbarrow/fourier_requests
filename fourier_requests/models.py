...
class CompositeModel(object):
    def __init__(self, historic_data, num_periodic_components):
        self.linear_model = LinearModel(historic_data)
        adjusted_historic_data = self._remove_linear_component(historic_data)
        self.fourier_model = FourierModel(adjusted_historic_data, num_periodic_components)

    def predict_at_time(self, true_time):
        time_in_period = self._adjust_time_to_period(true_time)
        return self.fourier_model.predict_at_time(time_in_period) + self.linear_model.predict_at_time(true_time)
    ...
    def _remove_linear_component(self, time_series):
        return [time_series[i] - self.linear_model.predict_at_time(i) for i in range(len(time_series))]
